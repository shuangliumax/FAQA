...
if __name__ == '__main__':
    ...
    from model.faster_rcnn.FAQA.resnet import resnet
    if args.net == 'vgg16':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        ############################################################################
        loss_rcnn_cls_temp_aug = 0
        loss_rcnn_box_temp_aug = 0
        loss_con_cls_temp = 0
        loss_con_loc_temp = 0
        loss_contra_temp = 0
        ############################################################################
        ...
        for step in range(1, iters_per_epoch + 1):
            ...
            rois_label, RCNN_loss_cls_aug, RCNN_loss_bbox_aug, consistency_loss_cls, \
            consistency_loss_loc, contrastive_loss = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)

            loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
                   + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() + \
            0.1 * RCNN_loss_cls_aug.mean() + 0.1 * RCNN_loss_bbox_aug.mean() + consistency_loss_cls + \
                   consistency_loss_loc + contrastive_loss
            ...
            loss_rcnn_cls_temp_aug += RCNN_loss_cls_aug.mean().item()
            loss_rcnn_box_temp_aug += RCNN_loss_bbox_aug.mean().item()
            loss_con_cls_temp += consistency_loss_cls.item()
            loss_con_loc_temp += consistency_loss_loc.item()
            loss_contra_temp += contrastive_loss.item()
            ...
            if step % args.disp_interval == 0:
                ...
                loss_rcnn_cls_temp_aug /= count_step
                loss_rcnn_box_temp_aug /= count_step
                loss_con_cls_temp /= count_step
                loss_con_loc_temp /= count_step
                loss_contra_temp /= count_step
                ...
                if args.mGPUs:
                    ...
                    ##############################################
                    loss_rcnn_cls_aug = RCNN_loss_cls_aug.item()
                    loss_rcnn_box_aug = RCNN_loss_bbox_aug.item()
                    loss_con_cls = consistency_loss_cls.item()
                    loss_con_loc = consistency_loss_loc.item()
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, rcnn_cls_aug: %.4f, rcnn_box_aug %.4f, con_cls %.5f, con_loc: %.4f, contra: %.4f" \
                    % (loss_rpn_cls_temp, loss_rpn_box_temp, loss_rcnn_cls_temp, loss_rcnn_box_temp, loss_rcnn_cls_temp_aug, loss_rcnn_box_temp_aug, loss_con_cls_temp, loss_con_loc_temp, loss_contra_temp))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
                        ...
                        'loss_rcnn_cls_aug': loss_rcnn_cls_temp_aug,
                        'loss_rcnn_box_aug': loss_rcnn_box_temp_aug,
                        'loss_contra': loss_contra_temp,
                        'loss_con_cls': loss_con_cls_temp,
                        'loss_con_loc': loss_con_loc_temp
                    }
        ...
        save_name = os.path.join(output_dir,
                                 'source_{}_session_{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset,
                                     args.session, epoch,
                                     step))
        save_checkpoint({
